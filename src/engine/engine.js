...
const NO_ENGINE = 'No engine yet. Prompt Forge can install the Claude CLI for you: open the gear and click Install beside Claude. Or sign in to a CLI you already have, or add an API key.';
...
function createEngine({ providers, config, secrets, log }) {
  ...
  async function detectOne(p, cfg) {
    ...
    return {
      ...
      models, defaults, installUrl: p.installUrl || null, installable: Boolean(p.installable), signIn: p.signIn || null,
    };
  }
  ...
  function state() {
    return {
      providers: detections.map((d) => ({
        ...
        models: d.models, defaults: d.defaults, installUrl: d.installUrl, installable: d.installable, signIn: d.signIn,
      })),
